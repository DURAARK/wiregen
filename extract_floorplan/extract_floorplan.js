...
AABB.prototype.center = function() { return new Vec2((this.bbmin.x+this.bbmax.x)/2.0,(this.bbmin.y+this.bbmax.y)/2.0); }
AABB.prototype.toString = function() { return this.bbmin.toString() + "--" + this.bbmax.toString(); }
//-----------------------------------------------------------------------------------
  ...
  for (var r in ROOMS)
  {
    ...
    scale(room.center);
      ...
      var rooms = svgContainer.selectAll("polygon")
                      ...
                      .attr("points",function(d) { return d.points.map(function(d) { return d.x + "," + d.y; }).join(" ") })
